#include "cgra_flow.h"
#include "camera_pipeline_shared_compute.h"
#include "camera_pipeline_shared_memory.cpp"

int main(int argc, char **argv) {
  std::vector<prog> prgs = { camera_pipeline_shared() };
  ...
  for (auto& prg : prgs) {
    ...
    while (i < args.size()) {
      if (args[i] == "opt") {
        ...
        // Run interconnect agnostic tb
        auto cgra = aha_flow_result(prg, dir);
