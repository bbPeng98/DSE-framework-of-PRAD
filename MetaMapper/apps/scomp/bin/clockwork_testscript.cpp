...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	std::vector<int16_t> hw_input_stencil_stream_tile(64*64);   idx=0;
	...
	std::vector<int16_t> hw_output_stencil_stream_tile(64*64);   idx=0;
	for (int l1 = 0; l1 < 64; l1++) {
	for (int l0 = 0; l0 < 64; l0++) {
		hw_uint<16> actual = hw_output_stencil_stream.read();
		int actual_lane = actual.extract<0, 15>();
		hw_output_stencil[((l1*64) + l0)] = (int16_t)(actual_lane);
