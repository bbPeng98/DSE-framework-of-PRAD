...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	std::vector<uint16_t> hw_input_stencil_stream_tile(3*64*64);   idx=0;
	for (int l2 = 0; l2 < 64; l2++) {
	for (int l1 = 0; l1 < 64; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_input_stencil[((l2*192) + ((l1*3) + l0))]));
		hw_input_stencil_stream.write(in_val);
		hw_input_stencil_stream_tile[idx] = hw_input_stencil[((l2*192) + ((l1*3) + l0))];  idx += 1;
	} } } 
	...
	hw_input_stencil_file.write(reinterpret_cast<const char *>(hw_input_stencil_stream_tile.data()),
		sizeof(hw_input_stencil_stream_tile[0]) * 3 * 64 * 64);
	...
	std::vector<int16_t> hw_output_stencil_stream_tile(3*58*58);   idx=0;
	for (int l2 = 0; l2 < 58; l2++) {
	for (int l1 = 0; l1 < 58; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> actual = hw_output_stencil_stream.read();
		int actual_lane = actual.extract<0, 15>();
		hw_output_stencil[((l2*174) + ((l1*3) + l0))] = (int16_t)(actual_lane);
		hw_output_stencil_stream_tile[idx] = hw_output_stencil[((l2*174) + ((l1*3) + l0))];  idx += 1;
	} } } 
