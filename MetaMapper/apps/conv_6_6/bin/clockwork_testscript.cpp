...
#include "unoptimized_conv_6_6.h"
...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	unoptimized_conv_6_6(
		hw_input_stencil_stream,
		hw_output_stencil_stream
	);

	// provision output buffer
	std::vector<uint16_t> hw_output_stencil_stream_tile(59*59);   idx=0;
	for (int l1 = 0; l1 < 59; l1++) {
	for (int l0 = 0; l0 < 59; l0++) {
		hw_uint<16> actual = hw_output_stencil_stream.read();
		int actual_lane = actual.extract<0, 15>();
		hw_output_stencil[((l1*59) + l0)] = (uint16_t)(actual_lane);
		hw_output_stencil_stream_tile[idx] = hw_output_stencil[((l1*59) + l0)];  idx += 1;
	} } 
