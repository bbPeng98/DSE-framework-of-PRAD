...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	uint16_t *hw_output_global_wrapper_stencil = (uint16_t*) mem_object_list[1]->host_ptr;
	...
	for (int l1 = 0; l1 < 192; l1++) {
	for (int l0 = 0; l0 < 256; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_input_stencil[((l1*256) + l0)]));
		hw_input_stencil_stream.write(in_val);
		hw_input_stencil_stream_tile[idx] = hw_input_stencil[((l1*256) + l0)];  idx += 1;
	} } 
	...
	unoptimized_camera_pipeline_2x2(
	...
	for (int l2 = 0; l2 < 184; l2++) {
	for (int l1 = 0; l1 < 248; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> actual = hw_output_global_wrapper_stencil_stream.read();
		int actual_lane = actual.extract<0, 15>();
		hw_output_global_wrapper_stencil[((l2*744) + ((l1*3) + l0))] = (uint16_t)(actual_lane);
		hw_output_global_wrapper_stencil_stream_tile[idx] = hw_output_global_wrapper_stencil[((l2*744) + ((l1*3) + l0))];  idx += 1;
	} } } 
