...
#include "unoptimized_conv_chain.h"
...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	unoptimized_conv_chain(
		hw_input_stencil_stream,
		hw_output_stencil_stream
	);

	// provision output buffer
	std::vector<uint16_t> hw_output_stencil_stream_tile(58*58);   idx=0;
	...
	for (int l0 = 0; l0 < 58; l0++) {
		...
		hw_output_stencil[((l1*58) + l0)] = (uint16_t)(actual_lane);
