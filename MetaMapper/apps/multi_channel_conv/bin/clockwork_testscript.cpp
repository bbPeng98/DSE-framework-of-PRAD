...
#include "unoptimized_multi_channel_conv.h"
...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	unoptimized_multi_channel_conv(
		hw_input_stencil_stream,
		hw_output_stencil_stream
	);

	// provision output buffer
	std::vector<uint16_t> hw_output_stencil_stream_tile(62*62*3);   idx=0;
	...
	for (int l0 = 0; l0 < 62; l0++) {
		...
		hw_output_stencil[((l2*3844) + ((l1*62) + l0))] = (uint16_t)(actual_lane);
		hw_output_stencil_stream_tile[idx] = hw_output_stencil[((l2*3844) + ((l1*62) + l0))];  idx += 1;
	} } } 
