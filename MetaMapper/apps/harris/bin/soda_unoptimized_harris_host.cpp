...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_padded16_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*0;
  const int op_hcompute_padded16_global_wrapper_stencil_read_BYTES_PER_PIXEL = 16 / 8;
  size_t op_hcompute_padded16_global_wrapper_stencil_read_size_bytes = op_hcompute_padded16_global_wrapper_stencil_read_BYTES_PER_PIXEL * op_hcompute_padded16_global_wrapper_stencil_read_pipe0_DATA_SIZE;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > op_hcompute_padded16_global_wrapper_stencil_read_pipe0(op_hcompute_padded16_global_wrapper_stencil_read_size_bytes);

  std::ofstream input_op_hcompute_padded16_global_wrapper_stencil_read("op_hcompute_padded16_global_wrapper_stencil_read.csv");
  for (int i = 0; i < op_hcompute_padded16_global_wrapper_stencil_read_pipe0_DATA_SIZE; i++) {
...
#ifdef __FLOAT_OUTPUT__
    ((uint16_t*) (op_hcompute_padded16_global_wrapper_stencil_read_pipe0.data()))[i] = bitcast<uint16_t, float>(val);
#else // __FLOAT_OUTPUT__
    ((uint16_t*) (op_hcompute_padded16_global_wrapper_stencil_read_pipe0.data()))[i] = val;
#endif // __FLOAT_OUTPUT__
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "unoptimized_harris_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer op_hcompute_padded16_global_wrapper_stencil_read_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, op_hcompute_padded16_global_wrapper_stencil_read_size_bytes, op_hcompute_padded16_global_wrapper_stencil_read_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, op_hcompute_padded16_global_wrapper_stencil_read_pipe0_ocl_buf));
