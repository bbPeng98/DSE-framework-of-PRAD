...
#include "unoptimized_brighten_and_blur.h"
...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	unoptimized_brighten_and_blur(
		hw_input_stencil_stream,
		hw_output_stencil_stream
	);

	// provision output buffer
	std::vector<uint16_t> hw_output_stencil_stream_tile(63*63);   idx=0;
	for (int l1 = 0; l1 < 63; l1++) {
	for (int l0 = 0; l0 < 63; l0++) {
		hw_uint<16> actual = hw_output_stencil_stream.read();
		int actual_lane = actual.extract<0, 15>();
		hw_output_stencil[((l1*63) + l0)] = (uint16_t)(actual_lane);
		hw_output_stencil_stream_tile[idx] = hw_output_stencil[((l1*63) + l0)];  idx += 1;
	} } 
