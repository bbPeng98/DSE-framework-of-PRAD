...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	int16_t *hw_input_stencil = (int16_t*) mem_object_list[0]->host_ptr;
	int16_t *hw_kernel_stencil = (int16_t*) mem_object_list[1]->host_ptr;
	...
	std::vector<int16_t> hw_input_stencil_stream_tile(8*30*30);   idx=0;
	for (int l2 = 0; l2 < 30; l2++) {
	for (int l1 = 0; l1 < 30; l1++) {
	for (int l0 = 0; l0 < 8; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_input_stencil[((l2*240) + ((l1*8) + l0))]));
		hw_input_stencil_stream.write(in_val);
		hw_input_stencil_stream_tile[idx] = hw_input_stencil[((l2*240) + ((l1*8) + l0))];  idx += 1;
	} } } 
	...
	hw_input_stencil_file.write(reinterpret_cast<const char *>(hw_input_stencil_stream_tile.data()),
		sizeof(hw_input_stencil_stream_tile[0]) * 8 * 30 * 30);
	hw_input_stencil_file.close();

	// provision input stream hw_kernel_stencil_stream
	std::vector<int16_t> hw_kernel_stencil_stream_tile(8*3*3*3);   idx=0;
	for (int l3 = 0; l3 < 3; l3++) {
	for (int l2 = 0; l2 < 3; l2++) {
	for (int l1 = 0; l1 < 3; l1++) {
	for (int l0 = 0; l0 < 8; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_kernel_stencil[((l3*72) + ((l2*24) + ((l1*8) + l0)))]));
		hw_kernel_stencil_stream.write(in_val);
		hw_kernel_stencil_stream_tile[idx] = hw_kernel_stencil[((l3*72) + ((l2*24) + ((l1*8) + l0)))];  idx += 1;
	} } } } 
	...
	hw_kernel_stencil_file.write(reinterpret_cast<const char *>(hw_kernel_stencil_stream_tile.data()),
		sizeof(hw_kernel_stencil_stream_tile[0]) * 8 * 3 * 3 * 3);
	hw_kernel_stencil_file.close();


	// invoke clockwork program
	unoptimized_resnet_pond_layer(
	...
	std::vector<int16_t> hw_output_stencil_stream_tile(28*28*3);   idx=0;
	for (int l2 = 0; l2 < 3; l2++) {
	...
	for (int l0 = 0; l0 < 28; l0++) {
		...
		hw_output_stencil[((l2*784) + ((l1*28) + l0))] = (int16_t)(actual_lane);
