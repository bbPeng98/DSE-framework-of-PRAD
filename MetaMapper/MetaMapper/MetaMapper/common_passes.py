...
class DagToPdfSimp(Visitor):
    def doit(self, dag: Dag):
        AddID().run(dag)
        self.plotted_nodes = {"global.my_PE_2input", "Input", "Output","PipelineRegister"}
        self.child_list = []
        self.graph = Digraph()
        self.run(dag)
        return self.graph
...
class CountPEs(Visitor):
    ...
    def generic_visit(self, node):
        Visitor.generic_visit(self, node)
        if hasattr(node, "node_name"):
            if node.node_name == "global.PE" or node.node_name == "global.my_PE_2input" or node.node_name == "global.my_PE_3input":
                self.res += 1
