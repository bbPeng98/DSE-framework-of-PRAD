...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	std::vector<uint16_t> hw_input_stencil_stream_tile(75*75);   idx=0;
	for (int l1 = 0; l1 < 75; l1++) {
	for (int l0 = 0; l0 < 75; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_input_stencil[((l1*75) + l0)]));
		hw_input_stencil_stream.write(in_val);
		hw_input_stencil_stream_tile[idx] = hw_input_stencil[((l1*75) + l0)];  idx += 1;
	} } 
	...
	hw_input_stencil_file.write(reinterpret_cast<const char *>(hw_input_stencil_stream_tile.data()),
		sizeof(hw_input_stencil_stream_tile[0]) * 75 * 75);
	hw_input_stencil_file.close();


	// invoke clockwork program
	unoptimized_laplacian_pyramid(
