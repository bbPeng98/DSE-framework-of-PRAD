...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*0;
  const int op_hcompute_hw_weight_global_wrapper_stencil_read_BYTES_PER_PIXEL = 16 / 8;
  size_t op_hcompute_hw_weight_global_wrapper_stencil_read_size_bytes = op_hcompute_hw_weight_global_wrapper_stencil_read_BYTES_PER_PIXEL * op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_DATA_SIZE;

  total_size_bytes += op_hcompute_hw_weight_global_wrapper_stencil_read_size_bytes;
  total_size_bytes_read += op_hcompute_hw_weight_global_wrapper_stencil_read_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0(op_hcompute_hw_weight_global_wrapper_stencil_read_size_bytes);
  ...
  std::ofstream input_op_hcompute_hw_weight_global_wrapper_stencil_read("op_hcompute_hw_weight_global_wrapper_stencil_read.csv");
  for (int i = 0; i < op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    float  val = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
#else // __FLOAT_OUTPUT__
    uint16_t val = (rand() % 256);
...
#ifdef __FLOAT_OUTPUT__
    input_op_hcompute_hw_weight_global_wrapper_stencil_read << val << std::endl;
#else // __FLOAT_OUTPUT__
    input_op_hcompute_hw_weight_global_wrapper_stencil_read << val << std::endl;
#endif // __FLOAT_OUTPUT__

#ifdef __FLOAT_OUTPUT__
    ((uint16_t*) (op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0.data()))[i] = bitcast<uint16_t, float>(val);
#else // __FLOAT_OUTPUT__
    ((uint16_t*) (op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0.data()))[i] = val;
#endif // __FLOAT_OUTPUT__
  }

  input_op_hcompute_hw_weight_global_wrapper_stencil_read.close();
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "unoptimized_conv_2input_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, op_hcompute_hw_weight_global_wrapper_stencil_read_size_bytes, op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_ocl_buf));

  uint64_t transfer_size = num_epochs*(0 / 1);
  OCL_CHECK(err, err = krnl_vector_add.setArg(3, transfer_size));

  std::cout << "Migrating memory" << std::endl;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({op_hcompute_hw_input_global_wrapper_stencil_read_pipe0_ocl_buf, op_hcompute_hw_weight_global_wrapper_stencil_read_pipe0_ocl_buf}, 0));
