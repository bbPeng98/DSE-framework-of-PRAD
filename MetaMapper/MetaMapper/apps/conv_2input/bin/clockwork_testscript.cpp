...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	uint16_t *hw_weight_stencil = (uint16_t*) mem_object_list[1]->host_ptr;
	...
	HWStream< hw_uint<16> > hw_weight_stencil_stream;
	...
	// provision input stream hw_weight_stencil_stream
	std::vector<uint16_t> hw_weight_stencil_stream_tile(3*3);   idx=0;
	for (int l1 = 0; l1 < 3; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_weight_stencil[((l1*3) + l0)]));
		hw_weight_stencil_stream.write(in_val);
		hw_weight_stencil_stream_tile[idx] = hw_weight_stencil[((l1*3) + l0)];  idx += 1;
	} } 
	ofstream hw_weight_stencil_file("bin/hw_weight_stencil.leraw", ios::binary);
	hw_weight_stencil_file.write(reinterpret_cast<const char *>(hw_weight_stencil_stream_tile.data()),
		sizeof(hw_weight_stencil_stream_tile[0]) * 3 * 3);
	hw_weight_stencil_file.close();


	// invoke clockwork program
	unoptimized_conv_2input(
		hw_input_stencil_stream,
		hw_weight_stencil_stream,
		hw_output_stencil_stream
	);

	// provision output buffer
	std::vector<uint16_t> hw_output_stencil_stream_tile(62*62);   idx=0;
	...
	for (int l0 = 0; l0 < 62; l0++) {
		...
		hw_output_stencil_stream_tile[idx] = hw_output_stencil[((l1*62) + l0)];  idx += 1;
	} } 
	...
	hw_output_file.write(reinterpret_cast<const char *>(hw_output_stencil_stream_tile.data()),
		sizeof(hw_output_stencil_stream_tile[0]) * 62 * 62);
