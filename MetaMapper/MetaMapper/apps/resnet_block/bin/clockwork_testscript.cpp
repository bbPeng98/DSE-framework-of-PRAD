...
void run_clockwork_program(RDAI_MemObject **mem_object_list) {
	...
	uint16_t *hw_kernel2_stencil = (uint16_t*) mem_object_list[2]->host_ptr;
	...
	HWStream< hw_uint<16> > hw_kernel2_stencil_stream;
	HWStream< hw_uint<16> > hw_output_stencil_stream;
	int idx = 0;

	// provision input stream hw_input_stencil_stream
	std::vector<uint16_t> hw_input_stencil_stream_tile(3*32*32);   idx=0;
	for (int l2 = 0; l2 < 32; l2++) {
	for (int l1 = 0; l1 < 32; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_input_stencil[((l2*96) + ((l1*3) + l0))]));
		hw_input_stencil_stream.write(in_val);
		hw_input_stencil_stream_tile[idx] = hw_input_stencil[((l2*96) + ((l1*3) + l0))];  idx += 1;
	} } } 
	...
	hw_input_stencil_file.write(reinterpret_cast<const char *>(hw_input_stencil_stream_tile.data()),
		sizeof(hw_input_stencil_stream_tile[0]) * 3 * 32 * 32);
	hw_input_stencil_file.close();

	// provision input stream hw_kernel_stencil_stream
	std::vector<uint16_t> hw_kernel_stencil_stream_tile(3*3*3*3);   idx=0;
	for (int l3 = 0; l3 < 3; l3++) {
	for (int l2 = 0; l2 < 3; l2++) {
	for (int l1 = 0; l1 < 3; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_kernel_stencil[((l3*27) + ((l2*9) + ((l1*3) + l0)))]));
		hw_kernel_stencil_stream.write(in_val);
		hw_kernel_stencil_stream_tile[idx] = hw_kernel_stencil[((l3*27) + ((l2*9) + ((l1*3) + l0)))];  idx += 1;
	} } } } 
	...
	hw_kernel_stencil_file.write(reinterpret_cast<const char *>(hw_kernel_stencil_stream_tile.data()),
		sizeof(hw_kernel_stencil_stream_tile[0]) * 3 * 3 * 3 * 3);
	hw_kernel_stencil_file.close();

	// provision input stream hw_kernel2_stencil_stream
	std::vector<uint16_t> hw_kernel2_stencil_stream_tile(3*3*3*3);   idx=0;
	for (int l3 = 0; l3 < 3; l3++) {
	for (int l2 = 0; l2 < 3; l2++) {
	for (int l1 = 0; l1 < 3; l1++) {
	for (int l0 = 0; l0 < 3; l0++) {
		hw_uint<16> in_val;
		set_at<0, 16, 16>(in_val, hw_uint<16>(hw_kernel2_stencil[((l3*27) + ((l2*9) + ((l1*3) + l0)))]));
		hw_kernel2_stencil_stream.write(in_val);
		hw_kernel2_stencil_stream_tile[idx] = hw_kernel2_stencil[((l3*27) + ((l2*9) + ((l1*3) + l0)))];  idx += 1;
	} } } } 
	ofstream hw_kernel2_stencil_file("bin/hw_kernel2_stencil.leraw", ios::binary);
	hw_kernel2_stencil_file.write(reinterpret_cast<const char *>(hw_kernel2_stencil_stream_tile.data()),
		sizeof(hw_kernel2_stencil_stream_tile[0]) * 3 * 3 * 3 * 3);
	hw_kernel2_stencil_file.close();


	// invoke clockwork program
	unoptimized_resnet_block(
		hw_input_stencil_stream,
		hw_kernel_stencil_stream,
		hw_kernel2_stencil_stream,
