...
import os
from archipelago.pnr_ import pnr_pack
#import archipelago.power
import archipelago.io
import math

#from peak_generator.peak_gen.peak_wrapper import wrapped_peak_class
#from peak_generator.peak_gen.arch import read_arch

import metamapper.coreir_util as cutil
#from metamapper.node import Nodes
#from metamapper.common_passes import VerifyNodes, print_dag, gen_dag_img
from metamapper import CoreIRContext
from metamapper.irs.coreir.__init__ import gen_CoreIRNodes
...
#               in bitstream]
#         f.write("\n".join(bs))
def load_netlist(pe_fc,harden_flush, height,pipeline_config_interval,app,pipeline_input_broadcasts,
                     input_broadcast_branch_factor, input_broadcast_max_leaves):
    #app_dir = os.path.dirname(app)    # return dir 
    app_dir = f"/home/dai-dirk/MetaMapper/MetaMapper"
    ...
    if pe_fc == lassen_fc:
        pe_header = f"{app_dir}/libs/lassen_header.json"
    elif pe_fc == lassen_fc_3:
        pe_header = f"{app_dir}/libs/lassen_header.json"
    elif pe_fc == lassen_fc_org:
        pe_header = f"{app_dir}/libs/lassen_header.json"
    else:
        pe_header = f"{app_dir}/libs/pe_header.json"

    mem_header = f"{app_dir}/libs/mem_header.json"
    io_header = f"{app_dir}/libs/io_header.json"
    bit_io_header = f"{app_dir}/libs/bit_io_header.json"
    pond_header = f"{app_dir}/libs/pond_header.json"
    print(pe_header)
    ...
    if(pe_fc == lassen_fc):
        putil.load_and_link_peak(
                nodes,
                pe_header,
                {"global.my_PE_2input": pe_fc},
            )
    elif(pe_fc == lassen_fc_3):
        putil.load_and_link_peak(
            nodes,
            pe_header,
            {"global.my_PE_3input":pe_fc},
        )
    elif(pe_fc == lassen_fc_org):
        putil.load_and_link_peak(
            nodes,
    ...
    if pe_fc == lassen_fc:
        tile_info = {"global.my_PE_2input": pe_fc, "global.MEM": MEM_fc,
                     "global.IO": IO_fc, "global.BitIO": BitIO_fc, "global.Pond": Pond_fc}
    elif pe_fc == lassen_fc_3:
        tile_info = {"global.my_PE_3input": pe_fc, "global.MEM": MEM_fc,
                     "global.IO": IO_fc, "global.BitIO": BitIO_fc, "global.Pond": Pond_fc}
    elif pe_fc == lassen_fc_org:
        tile_info = {"global.PE": pe_fc, "global.MEM": MEM_fc,
                     "global.IO": IO_fc, "global.BitIO": BitIO_fc, "global.Pond": Pond_fc}

    netlist_info = create_netlist_info(dag,
                                            tile_info,
                                            harden_flush,
                                           height//pipeline_config_interval,
...
def place_and_route(pe_fc,harden_flush, height,pipeline_config_interval, halide_src, 
                        pipeline_input_broadcasts=False, input_broadcast_branch_factor=4, input_broadcast_max_leaves=16):
        id_to_name, instance_to_instr, netlist, bus = load_netlist(pe_fc,harden_flush, height,pipeline_config_interval,halide_src,                                                       
                                                                        pipeline_input_broadcasts,
                                                                        input_broadcast_branch_factor,
                                                                        input_broadcast_max_leaves)
        ...
        id_to_name = pnr_pack((netlist, bus),   
                                                         cwd=app_dir,
...
def main(no_harden_flush,height,pipeline_config_interval,halide_src,\
    no_input_broadcast_pipelining,input_broadcast_branch_factor,input_broadcast_max_leaves):
    ...
    harden_flush=not no_harden_flush
    #height=args.height
    #pipeline_config_interval=args.pipeline_config_interval

    if len(halide_src) > 0 :      #5 paramters

        id_to_name, instance_to_instr, \
            netlist, bus = place_and_route(pe_fc,harden_flush, height,pipeline_config_interval,
                halide_src, pipeline_input_broadcasts=not no_input_broadcast_pipelining,
                input_broadcast_branch_factor=input_broadcast_branch_factor, input_broadcast_max_leaves=input_broadcast_max_leaves)
