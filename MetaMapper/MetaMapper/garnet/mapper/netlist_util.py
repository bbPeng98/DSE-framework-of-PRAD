...
from hwtypes.bit_vector import Bit, BitVector
...
class CountTiles(Visitor):
    ...
    def doit(self, dag: Dag):
        ...
        with open("/home/dai-dirk/AHA_GIB/Chisel/chisel-AHA9.0/src/main/resources/cgra_spec.json",mode = "r") as file_:
            arg = json.load(file_)
            num_row = arg["num_row"]
            num_colum = arg["num_colum"]

        pes = num_row * (num_colum - int(num_colum/4))
        mems = num_row * int(num_colum/4)
        ios = num_colum *2
        pe_ratio = (self.num_pes)/pes
        if mems !=0 :
            mem_ratio = (self.num_mems/2)/mems
        else:
            mem_ratio = 0
        io_ratio = self.num_ios/ios
        print(pe_ratio)
        print(mem_ratio)
        print(io_ratio)
        assert(pe_ratio<=1)
        assert(mem_ratio<=1)
        assert(io_ratio<=1)
        # file = open("/home/dai-dirk/AHA_GIB/benchmarks/tile_num.txt",mode="w")
        # file.write(json.dumps({"pe_ratio":pe_ratio,"mem_ratio":mem_ratio,"io_ratio":io_ratio}))
        # file.close()
        with open('/home/dai-dirk/AHA_GIB/benchmarks/tile_num_pe.txt', 'w') as p:   
            p.write(str(1-pe_ratio))
        with open('/home/dai-dirk/AHA_GIB/benchmarks/tile_num_mem.txt', 'w') as m:   
            m.write(str(1-mem_ratio))
        with open('/home/dai-dirk/AHA_GIB/benchmarks/tile_num_io.txt', 'w') as i:   
            i.write(str(1-io_ratio))
    ...
    def generic_visit(self, node: DagNode):
        ...
        if node.node_name == "Input" or node.node_name == "Output":
            self.num_ios += 1
        elif node.node_name == "global.PE" or node.node_name == "global.my_PE_2input" or node.node_name == "global.my_PE_3input":
            self.num_pes += 1
...
def create_netlist_info(
    ...
    def tile_to_char(t):
        if t.split(".")[1] == "PE" or t.split(".")[1] == "my_PE_2input" or t.split(".")[1] == "my_PE_3input":
            return "p"
        elif t.split(".")[1] == "MEM":
            return "m"
        elif t.split(".")[1] == "Pond":
