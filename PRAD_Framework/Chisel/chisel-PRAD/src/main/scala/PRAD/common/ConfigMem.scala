...
import PRAD.ir.IR
import PRAD.spec.CGRASpec.attrs
import chisel3._
 ...
 */ 
class ConfigMem(regWidth: Int, regNum: Int, cfgDataWidth: Int) extends Module with IR{
  apply(attrs)
  val cfgAddrWidth = getAttrValue("cfg_addr_width_Unit").asInstanceOf[Int] //8bit
  if(regNum > 1) {
    assert(cfgDataWidth % regWidth == 0)
  }
//  val cfgAddrWidth = log2Ceil((regWidth * regNum + cfgDataWidth - 1) / cfgDataWidth)//为什么要加一个cfgDataWidth
...
//object config_mem extends  App{
//
//  var test = (0 until 100).toList
//
//  Driver.emitVerilog(new ConfigMem(70,1,32))
//}
