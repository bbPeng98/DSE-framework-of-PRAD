...
class CgraParam(attrs: mutable.Map[String, Any]){
  ...
  val cfgAddrWidthOfUnit = attrs("cfg_addr_width_Unit").asInstanceOf[Int]
  val cfgBlkOffset = attrs("cfg_blk_offset").asInstanceOf[Int]   // configuration offset bit of blocks

  // ====== PE-Specific attributes =======//
  val PEinputnum = attrs("PEinputnum").asInstanceOf[List[Int]]
  val operations = attrs("operations").asInstanceOf[Map[String,Int]]
  val lut_n = attrs("lut_n").asInstanceOf[Int]
  val has_3input = attrs("has_3input").asInstanceOf[Int]

  // ====== MEM-Specific attributes =======//
  val interconnect_output_ports = attrs("interconnect_output_ports").asInstanceOf[Int]
  val mem_width = attrs("mem_width").asInstanceOf[Int]
  val mem_depth = attrs("mem_depth").asInstanceOf[Int]
  val use_sram_stub = attrs("use_sram_stub").asInstanceOf[Int]
  val target = attrs("target").asInstanceOf[List[Int]]
  val iterator_support = attrs("iterator_support").asInstanceOf[Int]
  val config_width = attrs("config_width").asInstanceOf[Int]
  val interconnect_input_ports = attrs("interconnect_input_ports").asInstanceOf[Int]
  val agg_addr_width = attrs("agg_addr_width").asInstanceOf[Int]
  val agg_iter_support = attrs("agg_iter_support").asInstanceOf[Int]
  val agg_range_width = attrs("agg_range_width").asInstanceOf[Int]
  val tb_range_width = attrs("tb_range_width").asInstanceOf[Int]
  val tb_addr_width = attrs("tb_addr_width").asInstanceOf[Int]
  val banks = attrs("banks").asInstanceOf[Int]
  val tb_height = attrs("tb_height").asInstanceOf[Int]
  val agg_height = attrs("agg_height").asInstanceOf[Int]
  val agg_rd_addr_gen_width = attrs("agg_rd_addr_gen_width").asInstanceOf[Int]
...
//  val gpe_posmap : mutable.Map[Tuple2[Int,Int],Int] = mutable.Map() // [(x, y), type-id]
  ...
  val numTrackCoarse = attrs("num_track_coarse").asInstanceOf[Int]
  val numTrackFine = attrs("num_track_fine").asInstanceOf[Int]
  ...
  val fcMap_coarse = attrs("connect_flexibility_coarse").asInstanceOf[mutable.Map[String, Int]]
  val fcMap_fine = attrs("connect_flexibility_fine").asInstanceOf[mutable.Map[String, Int]]
