...
params = [
    {'name' : 'ope0', 'type' : 'cat', 'categories' : ["Add", "NULL"]},
    {'name' : 'ope1', 'type' : 'cat', 'categories' : ["Mult0", "NULL"]},
    {'name' : 'ope2', 'type' : 'cat', 'categories' : ["MULADD", "NULL"]},
    {'name' : 'ope3', 'type' : 'cat', 'categories' : ["TAA", "NULL"]},
    {'name' : 'ope4', 'type' : 'cat', 'categories' : ["TAS", "NULL"]},
    {'name' : 'ope5', 'type' : 'cat', 'categories' : ["TSA", "NULL"]},
    {'name' : 'ope6', 'type' : 'cat', 'categories' : ["TSS", "NULL"]},
    {'name' : 'num_row', 'type' : 'int', 'lb' : 2, 'ub' : 2},
    {'name' : 'num_colum', 'type' : 'int', 'lb' : 6, 'ub' : 6},
    {'name' : 'PEinputnumFine', 'type' : 'int', 'lb' : 3, 'ub' : 3},
    {'name' : 'num_track_coarse',             'type' : 'int', 'lb' : 1,  'ub' : 5},
    {'name' : 'num_track_fine',               'type' : 'int', 'lb' : 1,  'ub' : 5}, 
    {'name' : 'num_otrack_per_opin_coarse',   'type' : 'int', 'lb' : 2,  'ub' :8 },
    {'name' : 'num_itrack_per_ipin_coarse',   'type' : 'int', 'lb' : 2,  'ub' :8 },
    {'name' : 'num_ipin_per_opin_coarse',     'type' : 'int', 'lb' : 2,  'ub' : 8  },
    {'name' : 'num_otrack_per_opin_fine',     'type' : 'int', 'lb' : 2,  'ub' : 8 },
    {'name' : 'num_itrack_per_ipin_fine',     'type' : 'int', 'lb' : 2,  'ub' : 8 },
    {'name' : 'num_ipin_per_opin_fine',       'type' : 'int', 'lb' : 2,  'ub' : 8}
]
...
#     {'name' : 'num_row', 'type' : 'int', 'lb' : 2, 'ub' : 2},
#     {'name' : 'num_colum', 'type' : 'int', 'lb' : 6, 'ub' : 6},
#     {'name' : 'PEinputnumFine', 'type' : 'int', 'lb' : 2, 'ub' : 3},
#     {'name' : 'num_track_coarse',             'type' : 'int', 'lb' : 4,  'ub' : 4},
#     {'name' : 'num_track_fine',               'type' : 'int', 'lb' : 2,  'ub' : 2}, 
#     {'name' : 'num_otrack_per_opin_coarse',   'type' : 'int', 'lb' : 8,  'ub' :8 },
#     {'name' : 'num_itrack_per_ipin_coarse',   'type' : 'int', 'lb' : 8,  'ub' :8 },
#     {'name' : 'num_ipin_per_opin_coarse',     'type' : 'int', 'lb' : 6,  'ub' : 6  },
#     {'name' : 'num_otrack_per_opin_fine',     'type' : 'int', 'lb' : 8,  'ub' : 8 },
#     {'name' : 'num_itrack_per_ipin_fine',     'type' : 'int', 'lb' : 8,  'ub' : 8 },
#     {'name' : 'num_ipin_per_opin_fine',       'type' : 'int', 'lb' : 8,  'ub' : 8}
# ]
...
def formatSpec(sample : dict) -> dict:
    ...
    for key in list(sample.keys()):
        if(key.find('op')!=-1):
            if(sample[key].find('NULL')!=-1):
                del sample[key]
            else:
                operations[sample[key]] = i
                i = i+1
                del sample[key]
...
#select benchmark
def run_cgra():
    detect1 = subprocess.run('cd ../ && ./pnr_new.sh "conv_3_3"', shell=True).returncode
    ...
    detect4 = subprocess.run('cp ../benchmarks/conv_3_3/parseroute.txt .',shell=True).returncode
    detect5 = subprocess.run('rm ../benchmarks/conv_3_3/parseroute.txt',shell=True).returncode
...
for i in range(it):
    ...
    spec = rec.to_dict('records')[0]
    argsjson["lut_n"] = spec["PEinputnumFine"]
    argsOpList = []
    for key in list(spec.keys()):
        if(key.find('ope')!=-1):
            argsOpList.append(spec[key])
    argsjson["op"] = argsOpList
    for key in list(spec.keys()):
        if(key.find('ope2')!=-1 or key.find('ope3')!=-1 or key.find('ope4')!=-1 or key.find('ope5')!=-1 or key.find('ope6')!=-1):
            spec['PEinputnumCoarse'] = 3
            argsjson["has_3input"] = spec["PEinputnumCoarse"] - 2
            break
        else:
            spec['PEinputnumCoarse'] = 2
            argsjson["has_3input"] = spec["PEinputnumCoarse"] - 2
...
if(resultKey.size!=0):
    while(1):
        ...
        argsOpList = []
        for key in list(beforeOpt['operations'].keys()):
            argsOpList.append(key)
        argsjson["op"] = argsOpList
        with open('../../MetaMapper/MetaMapper/src/lassen/lassen/args.json', 'w') as a:
            json.dump(argsjson, a, indent=4)
