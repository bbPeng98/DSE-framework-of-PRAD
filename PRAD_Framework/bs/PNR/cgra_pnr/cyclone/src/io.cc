...
void connect_nodes(Node &from, std::ifstream &in, RoutingGraph &g) {
    ...
    while (std::getline(in, line)) {
        ...
        if (line_tokens[0] == SwitchBoxNode::TOKEN) {
            ...
            // std::cout<<"from node is: "<<from.to_string()<<std::endl;
            // std::cout<<"port node is: "<<port.to_string()<<std::endl;
    ...
    while(std::getline(in, line)) {
        ...
        if (line_tokens[0] == Switch::TOKEN) {
            ...
            while (std::getline(in, line)) {
                ...
                if (line_tokens.size() != 4)
                    throw ::runtime_error("unable to process line" + line);
                uint32_t track_from = stou(line_tokens[1]);//according to GIB ifo(srcSide, srcTrack, dstSide, dstTrack), change the index.
                SwitchBoxSide side_from = gsi(stou(line_tokens[0]));
                uint32_t track_to = stou(line_tokens[3]);
                SwitchBoxSide side_to = gsi(stou(line_tokens[2]));
                // std::cout<<stou(line_tokens[0])<<" "<<stou(line_tokens[1])<<" "<<stou(line_tokens[2])<<" "<<stou(line_tokens[3])<<std::endl;
