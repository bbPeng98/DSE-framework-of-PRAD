...
uint32_t Node::get_edge_cost(const std::shared_ptr<Node> &node) {
    std::weak_ptr<Node> n = node;
    /*for(auto iter : neighbors_){
        std::cout << "neighbor: " << iter.lock()->to_string() << std::endl;
    }*/
...
Switch::Switch(uint32_t x, uint32_t y, uint32_t num_track,
    ...
    uint32_t num_track_spc = num_track;//uesd for track in different sides
    for (uint32_t side = 0; side < SIDES; side++) {
        if (side > 3) num_track_spc = 6;//because for diag side, there are 6 input ports for mem
        for (uint32_t io = 0; io < IOS; io++) {
            sbs_[side][io] = ::vector<shared_ptr<SwitchBoxNode>>(num_track_spc);
            for (uint32_t i = 0; i < num_track_spc; i++) {
                sbs_[side][io][i] =
...
void RoutingGraph::add_edge(const Node &node1, const Node &node2,
    ...
    //std::cout << n1->to_string() << " has neighbor: " << n2->to_string() << std::endl;
}

std::shared_ptr<Node> RoutingGraph::search_create_node(const Node &node) {
    uint32_t x = node.x;
    uint32_t y = node.y;
    // std::cout<<"unroutable node is: "<<node.to_string()<<std::endl;
