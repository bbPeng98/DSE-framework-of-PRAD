...
void GlobalRouter::route() {
    ...
    //     std::cout<<"net.id: "<<net_id<<std::endl;
    // }

    for (uint32_t it = 0; it < num_iteration_; it++) {
        auto time_start = std::chrono::system_clock::now();

        std::cout << "Routing iteration: " << ::setw(3) << it <<std::endl;;
    ...
    for (uint32_t pin_index = 0; pin_index < pin_indices.size(); pin_index++) {
        ...
        if (strategy == RoutingStrategy::CongestionDriven
            ...
            for (uint32_t p = 1; p < current_path.size(); p++) {
                ...
                // std::cout<<"seek src_node: "<<node->to_string()<<std::endl;
        ...
        if (sink_node.name[0] == 'r') {
            std::cout<<"src_node is: "<<src_node->to_string()<<std::endl;
            std::cout<<"sink_node is reg: "<<sink_node.name<<"  "<<sink_node.x<<", "<<sink_node.y<<'\n'<<std::endl;
            ...
            */
            ...
            auto segment = route_a_star(src_node, end_f, cost_f, h_f, net[0].name[0] == 'r');
            // for (const auto &node: segment){
            //     std::cout<<"end reg's result: "<<node->to_string()<<std::endl;
            // }
            ...
            std::cout<<"src_node is: "<<src_node->to_string()<<std::endl;
            std::cout<<"sink_node is not reg: "<<sink_node.name<<"  "<<sink_node.x<<", "<<sink_node.y<<'\n'<<std::endl;
            if (sink_node.node == nullptr)
                throw ::runtime_error("unable to find node for block"
                                      " " + sink_node.name);
            auto segment = route_a_star(src_node, sink_node.node, cost_f, net[0].name[0] == 'r');
            // for (const auto &node: segment){
            //     std::cout<<"end not reg's result: "<<node->to_string()<<std::endl;
            // }
...
GlobalRouter::create_cost_function(double an,
    ...
    return [&, an, it, net_id](const ::shared_ptr<Node> &node1,
        ...
        // std::cout<<"node1's simbol is: "<<node1->to_string().substr(0,3)<<std::endl;
        // std::cout<<"node2's simbol is: "<<node2->to_string().substr(0,3)<<std::endl;
        // std::cout<<"node1's side is: "<<node1->to_string().at(14)  - '0'<<std::endl;
        // std::cout<<"node2's side is: "<<node2->to_string().at(14)  - '0'<<std::endl;

        if(node1->to_string().substr(0,3) == "GIB" && (node1->to_string().at(14) - '0') > 3
            && node2->to_string().substr(0,3) == "GIB" && (node2->to_string().at(14) - '0') > 3){
                // std::cout<<"detedted pin2pin, half result!"<<std::endl;
                result /= 2;
        }
        return result;
    };
...
GlobalRouter::get_free_switch(const std::pair<uint32_t, uint32_t> &p) {
    return [&, p](const std::shared_ptr<Node> &node) -> bool {
        if (node == nullptr || node->type != NodeType::SwitchBox
            || node->x != p.first || node->y != p.second) {//even route successfully, condition is not right.
            // std::cout<<"if return false directly? "<<std::endl;
            return false;
        }
        else {
            // see it's been used or not
            if (!node_connections_.at(node).empty()){
                std::cout<<"have found the target node, "<<node->to_string()<<" but it's been used."<<std::endl;
                return false;
            }
...
void GlobalRouter::fix_register_net(int net_id, Pin &pin) {
    ...
    // std::cout<<"src_node: "<<src_node->to_string()<<std::endl;
    ...
    // for (const auto &node : segment) {
    //     std::cout<<"segment: "<<node->to_string()<<std::endl;
    // }

    for (const auto &node : segment) {
        for (const auto &next : *node) {//find next again!
            // std::cout<<"node is: "<<node->to_string()<<std::endl;
            // std::cout<<"next is: "<<next.lock()->to_string()<<std::endl;
    ...
    for (auto const &next_node : *reg_node) {
        for (index = 0; index < segment.size(); index++) {
            // std::cout<<"segment[index]: "<<segment[index]->to_string()<<" next_node.lock(): "<<next_node.lock()->to_string()<<std::endl;
