...
prefixed_placement(const std::map<std::string,
    ...
    // std::cout<<"placement_filename: "<<placement_filename.size()<<std::endl;
    std::map<std::string, std::pair<int, int>> result;
    if (use_prefix) {
        // std::cout<<"load_placement"<<std::endl;
    ...
    auto cmp = [](const ::string &a, const ::string &b) -> bool {//sort working_set
    ...
    for (const auto &iter: netlist) {
        for (auto const &blk : iter.second) {
            // std::cout<<"blk: "<<blk<<"  blk[0]:  "<<blk[0]<<std::endl;
            if ((blk[0] == 'i' || blk[0] == 'I')
                && result.find(blk) == result.end()){
                    std::cout<<"detect io: "<<blk<<std::endl;
                    working_set.insert(blk);
                }
        }
    }
    // for (const auto &iter: working_set) {
    //     std::cout<<"working_set: "<<iter<<std::endl;
    // }
    ...
    // std::cout<<"\n\n\n\navailable_pos.size(): "<<available_pos.size()<<"   working_set.size(): "<<working_set.size()<<std::endl;
    if (available_pos.size()*2 < working_set.size())//all IO pos should be twice than 16bit or 1bit.
        throw std::runtime_error("unable to assign all IO tiles");
    uint32_t pos_index_1bit = 1;
    uint32_t pos_index_16bit = 0;
    for (auto const &blk_id : blocks) {
        if (blk_id[0] == 'i'){
            auto const &pos = available_pos[pos_index_1bit++];
            result[blk_id] = pos;
        }else{
            auto const &pos = available_pos[pos_index_16bit++];
            result[blk_id] = pos;
        }
...
int main(int argc, char *argv[]) {
    ...
    // std::cout<<"layout_file: "<<layout_file.size()<<" netlist_file: "<<netlist_file.size()<<" result_filename: "<<result_filename.size();
    
    // std::ifstream in;
    // in.open(result_filename);
    // std::string line;
